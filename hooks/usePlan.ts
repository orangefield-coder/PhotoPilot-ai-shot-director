...
function getLocalPlans(): Record<string, { plan: Plan; meta: { id: string; created_at: string; shot_type: string; selfie_url: string; scene_url: string } }> {
  if (typeof window === 'undefined') return {}
  try {
...
function saveLocalPlan(id: string, plan: Plan, meta: { shot_type: string; selfie_url: string; scene_url: string }) {
...
export function usePlan(initialPlan?: Plan, planId?: string) {
  ...
  return { plan, setPlan, markComplete, completedCount, saving }
}
